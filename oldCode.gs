/**
 * This function retrieves all payments from thre payment spreadsheet and returns them as an array of the rows.
...
/**
 * This method selects the last validity date (column N) from the payments spreadsheet for a given user.
...
function sendMembershipRenewalEMail(user, validityEndDate) {
    ...
    if (secondaryEmailAddress == '') {
        for (var i = 0; i < user.emails.length; i++) {
            if ((user.emails[i].address != user.primaryEmail) && !user.emails[i].address.includes('@bestalumni.net.test')){
